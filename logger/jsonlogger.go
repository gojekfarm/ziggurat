...
func (l *JSONLogger) Error(message string, err error, kvs ...map[string]interface{}) {
	if err != nil {
		appendFields(l.logger.Err(err), kvs).Msg(message)
	}
}

func (l *JSONLogger) Fatal(message string, err error, kvs ...map[string]interface{}) {
	if err != nil {
		appendFields(l.logger.Fatal(), kvs).Msg(message)
	}
}

func NewJSONLogger(level string) *JSONLogger {
	loggerInst := zerolog.New(os.Stdout).With().Timestamp().Logger().Level(logLevelMapping[level])
