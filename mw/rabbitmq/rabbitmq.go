...
func RabbitRetrier(ctx context.Context, hosts []string, queueConfig QueueConfig, handler ziggurat.MessageHandler) *RabbitMQRetry {
	...
	if err := r.initPublisher(ctx); err != nil {
		ziggurat.LogFatal(err, "[RABBITMQ MW CONNECTION ERROR]", nil)
	}
	return r
}
...
func (r *RabbitMQRetry) HandleMessage(event ziggurat.MessageEvent, app ziggurat.App) ziggurat.ProcessStatus {
	...
	if status == ziggurat.RetryMessage {
		ziggurat.LogFatal(r.retry(event, app), "RABBITMQ RETRY MW", nil)
	}
	return status
}

func (r *RabbitMQRetry) Retry(handler ziggurat.MessageHandler) ziggurat.MessageHandler {
	return ziggurat.HandlerFunc(func(messageEvent ziggurat.MessageEvent, app ziggurat.App) ziggurat.ProcessStatus {
		...
		if status == ziggurat.RetryMessage {
			ziggurat.LogFatal(r.retry(messageEvent, app), "RABBITMQ RETRY MW", nil)
		}
		return status
	})
}
